...
<script type="text/javascript">
	...
	scheduler.showLightbox = function(id) {
		...
		if (ev.is_dbdata == null)
			selectTime();
		//if(ev.is_dbdata == null) $("#category").val(ev.subject = "none");
	...
	<nav class="navbar navbar-inverse">
		<div class="container-fluid">
			...
			<ul class="nav navbar-nav navbar-center">
				...
				<c:if
					test="${loginInfo.member_type=='Resident'||loginInfo.member_type=='Host'}">
					<li><a
			...
			<c:if test="${!empty loginInfo }">
				...
				<ul class="nav navbar-nav navbar-right">
					<li class="dropdown"><a class="dropdown-toggle"
						data-toggle="dropdown" href="#"> <!-- Manager, host, resident에 맞는 이미지들 넣으면 됌 -->
							<c:if test="${loginInfo.member_type=='Manager' }">
								<img
									src="${pageContext.request.contextPath}/resources/images/manager-icon.png"
									width="30" height="30" />
							</c:if> <c:if test="${loginInfo.member_type=='Host' }">
								<img
									src="${pageContext.request.contextPath}/resources/images/host-icon.png"
									width="30" height="30" />
							</c:if> <c:if test="${loginInfo.member_type=='Resident' }">
								<img
									src="${pageContext.request.contextPath}/resources/images/residents-icon.png"
									width="30" height="30" />
							</c:if> <c:if test="${loginInfo.member_type=='General' }">
								<img
									src="${pageContext.request.contextPath}/resources/images/general-icon.png"
									width="30" height="30" />
							</c:if> <c:if test="${!empty memberProfile.member_file_id }">
								<img
							...
							</c:if> ${loginInfo.member_id }님 (${loginInfo.member_type})<span
							class="caret"></span></a>
						<ul class="dropdown-menu">
							...
							<c:if test="${loginInfo.member_type=='General' }">
								<li><a href="javascript:void(0)" onclick="insertHouse();">하우스
										등록</a>
							</c:if>
						</ul></li>
				</ul>
		...
		<div style="margin-left: 15%; padding: 1px 16px; height: 100%;">
			...
			<div id="my_form">
				<table>
					<tr id="">
						...
						<!-- css 수정 (style부분 추가)-->
						<td width="300px;"><select id="category"
					...
					<tr>
						...
						<td colspan="3"><input type="text" id="description"></td>
					</tr>
					<tr>
					</tr>
					<tr>
						<th style="padding: 6;">내용</th>
						<td colspan="3"><textarea id="content" rows="5" cols="50"></textarea></td>
					</tr>
					<tr>
            ...
            ${i }
					...
					<tr id="alertTr">
						...
						<td colspan="3"><select class="sel" id="alarm"
				...
				<div style="text-align: center;">
						...
						<input
						type="button" name="voteyes" value="모임 참여" id="voteyes"
						style='width: 100px;' onclick="vote_event('yes')">
						<input
						type="button" name="voteno" value="모임 불참" id="voteno"
						style='width: 100px;' onclick="vote_event('no')">
						<!--모임의 참여 및 불참여 버튼  -->
					<input type="button" name="save" value="저 장" id="save"
						style='width: 100px;' onclick="save_form()"> <input
						type="button" name="close" value="닫 기 " id="close"
